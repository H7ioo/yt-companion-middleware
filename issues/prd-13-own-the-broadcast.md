...
- **STATUS 2026-08-29 — the premise is VERIFIED; the note below is superseded.** The 2026-08-05
  failure has a mundane cause, confirmed by the operator: the scheduled broadcast was bound to a
  different stream key than the one OBS was pushing to, so YouTube minted a broadcast to receive
  video nothing was expecting. Re-tested with the event correctly attached to the reusable key: the
  scheduled broadcast aired, with Studio closed **and** with Studio open. What remains untested is a
  broadcast *this app* created — that is Test 3, the prerequisite in
  [PRD-16](prd-16-broadcast-list-and-scheduling.md), which supersedes this PRD's open questions and
  adds scheduling (explicitly out of scope here).
- **The central premise is unverified, and there is evidence against it.** _(Superseded — see the
  status line above. Kept for the record.)_ This PRD assumes that a broadcast we create and bind to the operator's reusable stream is the one the encoder feeds. The live test of 2026-08-05 23:01 shows YouTube minting its own broadcast (`3u1qu4bOxxM`) on that same stream key and airing it, while a valid `upcoming` broadcast already existed on the key. If YouTube does that regardless of what we create, our broadcast loses the race exactly as the stale one did and this migration buys nothing at considerable cost. **Probe before building:** insert a broadcast with `enableAutoStart`, bind it to the existing stream, start the encoder, and observe which one goes live — with YouTube Studio closed and again with it open. Confirm `liveBroadcasts.insert` is permitted on the channel in the same pass. Two API calls and one go-live either justifies this PRD or kills it.
- Sequencing: PRD-12 ships first and independently. It removes today's pain in a small diff. PRD-14 (adaptive polling while a latch is armed) is the cheap follow-up that shrinks the remaining window to a few seconds, and should ship regardless of how this PRD is decided. This is the largest change and should block neither.
