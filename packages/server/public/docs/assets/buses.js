  ...
  },
  {
    ...
    endpoints: [
      ...
      { m:"POST", path:"/api/dashboard/fill-request", cost:"local", auth:false,
        desc:"Raise a fill request for a preset (the Companion <em>Request fill</em> action). Surfaced as <code>fillRequest</code> on the state push so an open dashboard pops the fill dialog; with an ntfy topic configured the phone is also notified. Single slot — a new request replaces a pending one; unclaimed requests expire after 60s. Always HTTP 200, success/error in the body.",
        body:[ {k:"presetId", t:"text", req:true, hint:"preset id"} ] },
      { m:"POST", path:"/api/dashboard/fill-request/:id/claim", cost:"local", auth:false,
        desc:"Claim the pending fill request by id. Exactly one caller gets <code>claimed:true</code> and shows the fill dialog; the rest stay quiet.",
        pathParams:[ {k:"id", req:true} ] },
